"""
EuroStoxx recommendation backtest.

Expected inputs:
- EuroStoxx_signals_with_recommendations.csv
- EuroStoxx_clean.csv if available
- otherwise the raw EuroStoxx contract files in the expected data folder
...
How to use:
- run the EuroStoxx cleaning notebook first so it creates the recommendation file
...
DATA_SUBDIR = Path(r"data/EuroStoxx")
SIGNALS_FILENAME = "ES_signals_with_recommendations.csv"
RAW_GLOB = "VG*.csv"
CLEAN_CANDIDATES = ['ES_clean.csv', 'EuroStoxx_clean.csv', 'eurostoxx_clean.csv']
...
def save_tables(
    ...
    checks["missing_summary"].to_csv(output_dir / "EuroStoxx_recommendation_missing_summary.csv", index=True)
    checks["bad_buys"].to_csv(output_dir / "EuroStoxx_bad_buys.csv", index=False)
    checks["bad_sells"].to_csv(output_dir / "EuroStoxx_bad_sells.csv", index=False)
    checks["bad_tick_rows"].to_csv(output_dir / "EuroStoxx_bad_tick_rows.csv", index=False)

    market_tau.to_csv(output_dir / f"EuroStoxx_{tau_min}min_from_clean_or_raw.csv", index=False)
    merged.to_csv(output_dir / "EuroStoxx_backtest_results.csv", index=False)
...
def make_charts(output_dir: Path, summaries: dict[str, pd.DataFrame]) -> None:
    ...
    if "by_direction" in summaries and not summaries["by_direction"].empty:
        ...
        plt.title("EuroStoxx Average Improvement by Direction")
        plt.xlabel("Signal Direction")
        plt.ylabel("Average Improvement")
        plt.tight_layout()
        plt.savefig(output_dir / "EuroStoxx_avg_improvement_by_direction.png", dpi=200)
        plt.close()
    ...
    if "calibration" in summaries and not summaries["calibration"].empty:
        ...
        if not df.empty:
            ...
            plt.title("EuroStoxx Estimated vs Actual Fill Probability")
            plt.xlabel("Estimated Fill Probability")
            plt.ylabel("Actual Fill Rate")
            plt.tight_layout()
            plt.savefig(output_dir / "EuroStoxx_fill_probability_calibration.png", dpi=200)
            plt.close()
    ...
    if "by_ticks" in summaries and not summaries["by_ticks"].empty:
        ...
        plt.title("EuroStoxx Fill Rate by Ticks Away")
        plt.xlabel("Ticks Away")
        plt.ylabel("Fill Rate")
        plt.tight_layout()
        plt.savefig(output_dir / "EuroStoxx_fill_rate_by_ticks.png", dpi=200)
        plt.close()
...
def write_run_report(
    ...
    lines = []
    lines.append("EuroStoxx Backtest Run Report")
    lines.append("=" * 26)
    lines.append("")
    ...
    (output_dir / "EuroStoxx_backtest_run_report.txt").write_text("\n".join(lines), encoding="utf-8")
